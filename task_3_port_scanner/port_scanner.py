...
def check_port(ip, port, check_udp=False, check_protocol=False):
    check_tcp_res = check_port_tcp(ip, port, check_protocol)
    if check_tcp_res[0]:
        with LOCK_PRINTING:
            if check_tcp_res[1] != '':
    ...
    if check_udp:
        check_udp_res = check_port_udp(ip, port, check_protocol)
        if check_udp_res[0]:
            with LOCK_PRINTING:
                if check_udp_res[1] != '':
...
def check_port_tcp(ip, port, check_protocol=False) -> (bool, str):
...
def check_port_udp(ip, port, check_protocol=False) -> (bool, str):
    ...
    for i in range((end - start) // 1000 + 1):
        ...
        for port in range(_start, _end + 1):
            thread = threading.Thread(
                target=check_port,
                args=(host, port, args.udp, args.check_protocols))
